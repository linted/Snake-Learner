...
class QLearner(object):
    def __init__(
        ...
        self.Tc = (
            np.full((self.num_states, self.num_actions, self.num_states), 1e-10)
            if dyna
            else None
        )  # init'd really small so that we don't get div by zero errors
    ...
    def training_query(self, s_prime, r):
        ...
        self.Q[self.s, self.a] = ((1 - self.alpha) * self.Q[self.s, self.a]) + (
            self.alpha * (r + (self.gamma * self.Q[s_prime, next_action]))
        )
        if self.verbose:
            print("<s,a,s',r> = <{},{},{},{}> ".format(self.s, self.a, s_prime, r))
        ...
        if self.dyna:
            dyna_random = np.stack(
                [
                    np.random.randint(0, self.num_states, self.dyna),
                    np.random.randint(0, self.num_actions, self.dyna),
                ],
                axis=1,
            )
            for (dyna_s, dyna_a) in dyna_random:
                ...
                if self.verbose:
                    print(
                        "dyna tuple = <{},{},{},{}>".format(
                            dyna_s, dyna_a, dyna_s_prime, dyna_r
                        )
                    )
                p1 = (1 - self.alpha) * self.Q[dyna_s, dyna_a]
                p2 = self.alpha * (
                    dyna_r
                    + (self.gamma * self.Q[dyna_s_prime, np.argmax(self.Q[dyna_s])])
                )
